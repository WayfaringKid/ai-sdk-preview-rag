...
# **Question 1**: When arrays are passed to functions, are they passed by reference or as a pointer?
...
# **Question 1**: I am running public tests on my processing.cpp file, and am getting this error. I tried using the debugger but am still not able to find what could be wrong. What could I do to figure it out?    "Testing dog seam carve 4x5...zsh: segmentation fault ./processing_public_tests.exe"
...
# **Question 3**: Do I print both these error messages for any of the errors or do I print the error message that relates to the error?

**Answer**: for all errors, you will print the entire message, and then exit or return with a nonzero number.


# **Question 3**: Are comments graded?

**Answer**: No you don't need to worry about comments


# **Question 3**: When I try to submit to the autogravder, it says the due date has passed, but I though the due date was moved to tomorrow?
...
# **Question 4**: Hi,   My partner's and my code is failing one test out of all of the processing public tests. I know that crabster can catch more edge cases (it's briefly mentioned in FAQ) but I am struggling to find out which function is the root of the issue. We are passing all of the other tests (for dog, crabster, and horses) so I am hesitant to edit any of those functions. My most recent autograder submission is the code I have right now.  I found the exact pixel where our completed image deviates from the expected image (after carving), but I am not sure what to do with that information after looking over all of the functions called that I had thought could have affected the outcome.   At the pixel at row 1 and column 14 (after carving, not before), our image has values of 255 254 253, but the correct pixel has values of 254 255 253. At first, I thought this might have meant the red and blue values got switched at some point, but I looked at the crabster ppm files and found that (before carving) the pixel in the spot directly below row 1 column 14 has the correct value that is supposed to be found at row 1 column 14 (after carving). One reason I can think this might be happening is that the seam didn't pick the leftmost value (after spinning 90 degrees via carve_height) with the lowest cost, but I am confused because the public find seam and remove seam tests are not failing.   If it is passing those tests and failing seam carve, does that mean that the issue must be in seam carve or either of the two seam carve functions? Do you know where/what the issue might be?   Sorry for the long message, I wanted to be thorough.  Thank you!
...
# **Question 4**: Hi! Will the solutions to the lab 4 delivery problem be posted?   Thank you!

**Answer**: The solutions for lab 4 are available on google drive!


# **Question 4**: I understand that this is a very fast paced course with relatively fewer students during the spring, but I have been struggling to connect with my peers, whether in lecture or lab, despite my efforts to get to know them.  Are there any resources, clubs or things I can do to be better integrated and connected with this community

**No answers provided**


# **Question 4**: for the implementation of card_less, does the rank of the led card matter? or is only the suit important? thanks!
...
# **Question 7**: What does this format mean?
